...
namespace GridNavPath
{
    ...
    public class Space
    {
        ...
        public void link(Space other)
        {
            ...
            {
                if (links.ContainsKey(other.getId()))
                {
                    return;
                }
                float linkx = x;
         ...
         */
        private bool inTolerance(int a, int b)
        {
            return a == b;
        }
         ...
         */
        public bool hasJoinedEdge(Space other)
        {
            // aligned vertical edges
            if (inTolerance(x, other.x + other.width) || inTolerance(x + width, other.x))
            {
        ...
        public Space merge(Space other)
        {
            ...
            Space newSp = new Space((int)minx, (int)miny, (int)newwidth, (int)newheight);
            newSp.setId(getId());
            return newSp;
        }
         ...
         */
        public bool canMerge(Space other)
        {
    ...
    //        LogManager.Log("===========11=============", LogType.Normal);
     ...
     //           LogManager.Log("===========13=============", LogType.Normal);
         ...
         */
        public bool pickLowestCost(Space target, NavPath path)
        {
            if (target == this)
            {
                return true;
     ...
     //       LogManager.Log("===========21=============", LogType.Normal);
            Link bestLink = null;
            for (int i = 0; i < getLinkCount(); i++)
            {
                Link link = getLink(i);
     ...
     //       LogManager.Log("===========22=============", LogType.Normal);
            path.push(bestLink);
     //       LogManager.Log("===========23=============", LogType.Normal);
