...
namespace Lite.AStar
{
	public class Graph3DAStarMap : GraphAStarMap
	{
		...
		public TwVector3 RayCast3D(TwVector3 from, TwVector3 to)
		{
			...
			{
				a_xz = (float)dz / (float)dx;
				a_y = (float)dy / (float)dx;
				...
				for (long x = from.x + step; step > 0 ? x < to.x + step : x > to.x - step; x += step)
				{
					...
					float z = (float)from.z + a_xz * (float)(x - from.x);
					float y = (float)from.y + a_y * (float)(x - from.x);
			...
			{
				a_xz = (float)dx / (float)dz;
				a_y = (float)dy / (float)dz;
				...
				for (long z = from.z + step; step > 0 ? z < to.z + step : z > to.z - step; z += step)
				{
					...
					float x = (float)from.x + a_xz * (float)(z - from.z);
					float y = (float)from.y + a_y * (float)(z - from.z);
