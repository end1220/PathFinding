...
namespace Pathfinding
{
		...
		, IRaycastableGraph
	{
		...
		public static NNInfo GetNearest(NavMeshGraph graph, GraphNode[] nodes, Vector3 position, NNConstraint constraint, bool accurateNearestNode)
		{
			...
			if (query.node == null)
			{
				for (int i = 1; i <= 8; i++)
				{
					...
					if (query.node != null || (i - 1) * (i - 1) * w > 100 * 2)
					{ // *2 for a margin
						break;
					}
				}
			...
			{
				if (constraint.constrainDistance && ((Vector3)query.constrainedNode.position - position).sqrMagnitude > 10000)
				{
		...
		public static NNInfo GetNearestForceBoth(NavGraph graph, INavmeshHolder navmesh, Vector3 position, NNConstraint constraint, bool accurateNearestNode)
		{
			...
			float maxDistSqr = constraint.constrainDistance ? 10000 : float.PositiveInfinity;
		...
		public override void ScanInternal(OnScanStatus statusCallback)
		{
			...
			TriangleMeshNode.SetNavmeshHolder(0, this);
