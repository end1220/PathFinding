namespace PathFinding.RVO
{
	...
	public class Simulator
	{
		...
		public ObstacleVertex AddObstacle(Int3 a, Int3 b, Int height)
		{
			...
			vertex2.ignore = true;
			Int2 num = new Int2(b.x - a.x, b.z - a.z);
			num.Normalize();
			item.dir = num;
			vertex2.dir = -num;
			if (this.Multithreading && this.doubleBuffering)
		...
		private void BuildQuadtree()
		{
			...
			if (this.agents.Count > 0)
			{
				...
				for (int i = 1; i < this.agents.Count; i++)
				{
					...
					r = VRect.MinMaxRect(Mathf.Min(r.xMin, position.x), Mathf.Min(r.yMin, position.z), Mathf.Max(r.xMax, position.x), Mathf.Max(r.yMax, position.z));
				}
