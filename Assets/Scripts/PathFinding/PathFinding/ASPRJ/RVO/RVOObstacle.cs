namespace PathFinding.RVO
{
    ...
    public abstract class RVOObstacle : MonoBehaviour
    {
        ...
        protected void AddObstacle(Int3[] vertices, int height)
        {
            ...
            {
                Int3[] destinationArray = new Int3[vertices.Length];
        ...
        public void OnDrawGizmos(bool selected)
        {
            ...
            {
                Int3[] numArray = this.gizmoVerts[i];
                int index = 0;
                for (int j = numArray.Length - 1; index < numArray.Length; j = index++)
                {
                    Gizmos.DrawLine(matrix.MultiplyPoint3x4((Vector3) numArray[index]), matrix.MultiplyPoint3x4((Vector3) numArray[j]));
                }
                if (selected)
                {
                    ...
                    for (int k = numArray.Length - 1; num5 < numArray.Length; k = num5++)
                    {
                        ...
                        Gizmos.DrawLine(matrix.MultiplyPoint3x4((Vector3) numArray[num5]), matrix.MultiplyPoint3x4((Vector3) numArray[num5]) + ((Vector3) (Vector3.up * num4)));
                    }
                    int num7 = 0;
                    for (int m = numArray.Length - 1; num7 < numArray.Length; m = num7++)
                    {
                        ...
                        Vector3 vector4 = (vector2 - vector).normalized;
                        if (vector4 != Vector3.zero)
                        {
                            Vector3 vector5 = Vector3.Cross(Vector3.up, vector4);
                            Gizmos.DrawLine(vector3, vector3 + vector5);
                            Gizmos.DrawLine(vector3 + vector5, (Vector3) ((vector3 + (vector5 * 0.5f)) + (vector4 * 0.5f)));
                            Gizmos.DrawLine(vector3 + vector5, (Vector3) ((vector3 + (vector5 * 0.5f)) - (vector4 * 0.5f)));
                        }
                    }
                }
